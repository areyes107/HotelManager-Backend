                        ...
                        bcrypt.compare(params.password, check.password, (err, passworOk)=>{
                            if(err){
                                ...
                                if(params.gettoken = true){
                                    res.send({token: jwt.createTokenHotel(check), hotel: check.data.name});
                                }else{
...
function findAll(req, res){
    Hotel.find({}, {password:0, cuh:0}, (err, response)=>{
        if (err) {
            res.status(500).send({ message: "Error del servidor" });
        } else if (response) {
...
function findNameHotel(req, res){
    var params = req.body;
    Hotel.find({'data.name': {$regex: params.search}}, (err, hotel)=>{
        if (err) {
            res.status(500).send({ message: "Error del servidor" });
        } else if (hotel) {
